...
import {aiExtractionOfInformation, convertPdfToHtml, extractContainerNumbers} from '../utils/utils';
...
export async function parseFileContainers(req: Request, res: Response) {
    ...
    try {
        ...
        } else if (localFilePath.endsWith('.txt')) {
        ...
        const containers = await extractContainerNumbers(text);
...
`;
        instructions += containersCount > 0 ? ` Expect ${containersCount} containers in total. Look for information regarding these containers: ${containers.join('\n')}` : '';
